...
def system_check():
    ...
    for i in range(1, 101):  # Simulate progress with numbers
        ...
        time.sleep(0.05)  # Simulate time delay for each step
    print("\n")
...
if __name__ == "__main__":
    ...
    file_name = "TIMES LOGGED IN (DON'T DELETE).txt"
    try:
        with open(file_name, "r") as file:
            login_count = int(file.read())
    except FileNotFoundError:
        login_count = 0  # Default to 0 if the file doesn't exist
    except Exception as e:
        print(f"An unexpected error occurred: {e}")
        login_count = 0
        # Check if the variable is 0 and start PREPARE if needed():
        if login_count == 0:
            print("Starting TS-DOS Prepare....")
            for step in range(100):
                print("#", end="", flush=True)
                time.sleep(0.05)

            print(" Welcome To Prepare!")
            print("Prepare Will Prepare TS-DOS On cmd.exe")
            time.sleep(2)
            print("Preparing TS-DOS.....")
            for step in range(100):
             sys.stdout.write("#")
             sys.stdout.flush()  # Ensure the output appears immediately
             # Adjust the speed dynamically based on system performance
             time.sleep(0.05)  # Delay for demonstration, can be adjusted

            print()  # Move to the next line when the bar is complete

            print("TS-DOS Will Restart In 50000 Milliseconds")
            for step in range(100):
                print("#", end="", flush=True)
                time.sleep(0.05)
            ...
            if system_check():
                ...
                print(f"Welcome Back! You Have Logged In {login_count}")
    ...
    if system_check:
        ...
        print("EXITIING TS-DOS...") # Print the progress bar dynamically
        time.sleep(1)
        for step in range(100):
            print("#", end="", flush=True)
            time.sleep(0.05)
    ...
    new_content = login_count

    with open(".\\TIMES LOGGED IN (DON'T DELETE).txt", "w") as file:
        file.write(new_content)  # Write the variable's value into the file
